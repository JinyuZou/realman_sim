...
class H5Writer:
    ...
    root/qpos             (T, nq)
    root/qvel             (T, nv)
    root/observations/effort     (T, na)  # 若无则维度为 0
    root/observations/images/cam_high  (T, H, W, 3)   # top_cam
    root/observations/images/cam_left  (T, H, W, 3)   # wrist_cam_left
    root/observations/images/cam_low   (T, H, W, 3)   # button_cam
    root/observations/images/cam_right (T, H, W, 3)   # wrist_cam_right
    """
    ...
    def append(self, imgs_dict, action, qpos, qvel, effort):
        ...
        if self.ds_effort.shape[1] == 0:
            ...
            self.ds_effort[t, :] = (effort if effort is not None else
                                    np.zeros((self.ds_effort.shape[1],), np.float32)).astype(np.float32)
...
def main():
    ...
    try:
        while True:
                ...
                if (_pose_reached(l_now, initial_L) and _pose_reached(r_now, initial_R)) \
                   or ((time.time() - return_start_t) > MAX_RETURN_SECS):
                    ...
                    if first_img is not None:
                        ...
                        H, W = CAMERAS["top_cam"][0], CAMERAS["top_cam"][1]
            ...
            # ===== 写入 HDF5：定长 N 帧 =====
            if recording and (h5 is not None) and (not returning):
